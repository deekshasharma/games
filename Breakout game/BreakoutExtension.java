...
public class BreakoutExtension extends GraphicsProgram 
{
	...
	/* Control the ball movements when the ball hits the paddle, left wall, right wall and top wall and the floor   */
	...
	private void playGame()
	{
		...
		while(true)
		{
			...
			else if (ballHitFloor())
			{
				if (turnUsed < NTURNS ) 
				{	
					displayMessage(NTURNS - turnUsed + "  more turn remaining");
					turnUsed += 1;
					remove(ball);
					drawBall();
					pause(500);
	...
	/* Break the bricks in the path of the ball*/
